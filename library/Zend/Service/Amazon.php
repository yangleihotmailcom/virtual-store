 ...
 //       var_dump($response->getBody());exit();  
    ...
    public function itemLookup($asin, array $options = array())
    {
       ...
       // var_dump($response->getBody());exit();
        ...
        $dom = new DOMDocument();
        $dom->loadXML($response->getBody()); //print_r($dom->;exit();
        self::_checkErrors($dom);
        $xpath = new DOMXPath($dom); //print_r($xpath);exit();
...
//         	        exit;
    ...
    public function createCart( $options )
    {
    	$client = $this->getRestClient();
    	$client->setUri($this->_baseUri);
    	$client->getHttpClient()->resetParameters();
    	...
    	$options = $this->_prepareOptions('CartCreate', $options, $defaultOptions);
    	$response = $client->restGet('/onca/xml', $options);
    	if ($response->isError()) {
    		/**
    		 * @see Zend_Service_Exception
    	...
    	$dom = new DOMDocument();
    	$dom->loadXML($response->getBody()); //print_r($response->getBody());
    	self::_checkErrors($dom);
    	require_once 'Zend/Service/Amazon/RemoteCart.php';
    	return new Zend_Service_Amazon_RemoteCart($dom);
    }
